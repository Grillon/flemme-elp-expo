(function(){const o=document.createElement("link").relList;if(o&&o.supports&&o.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))i(r);new MutationObserver(r=>{for(const s of r)if(s.type==="childList")for(const n of s.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&i(n)}).observe(document,{childList:!0,subtree:!0});function t(r){const s={};return r.integrity&&(s.integrity=r.integrity),r.referrerPolicy&&(s.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?s.credentials="include":r.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function i(r){if(r.ep)return;r.ep=!0;const s=t(r);fetch(r.href,s)}})();const d="Thierry Vogel – Études de cas infrastructure",c="Architecte Système orienté infrastructure, fiabilité et automatisation chez Orange (2009–2023), aujourd’hui indépendant.",p=`De 2011 à 2015, j’ai occupé un rôle transverse d’automatisation et de fiabilisation chez Orange, en tant qu’ingénieur systèmes spécialisé HPUX et RHEL. Je m’appuyais sur BladeLogic pour créer des workflows agnostiques, robustes et maintenables.
...
`).map(t=>`<p class="text-blue-100 leading-relaxed">${t}</p>`).join("")}
  ...
  `}function y(e){let o='<div class="p-6 space-y-6 bg-slate-900/20">';return(e.contexte||e.declencheur)&&(o+=`
      <div>
...
`).map(r=>r.startsWith("- ")?`
            <div class="flex items-start gap-2 ml-4">
              ...
              <span class="text-blue-100">${r.substring(2)}</span>
            </div>
          `:r.trim()?`<p class="text-blue-100 leading-relaxed">${r}</p>`:"").join("")}</div>`:`<p class="text-blue-100 leading-relaxed">${t}</p>`).join("")}
        </div>
      </div>
    </section>
  `}function q(e){return`
  ...
  `}document.addEventListener("DOMContentLoaded",function(){const e=document.getElementById("app");e.innerHTML=f(h)+I(),L()});function I(){return`
  ...
  `}function L(){const e=document.querySelectorAll(".nav-item"),o=["header","resumes-rapides","a-propos","etudes-de-cas","aujourdhui","footer"];function t(){const i=window.scrollY+100;let r="header";o.forEach(s=>{const n=document.getElementById(s);if(n){const a=n.offsetTop,u=a+n.offsetHeight;i>=a&&i<u&&(r=s)}}),e.forEach(s=>{s.getAttribute("data-section")===r?(s.classList.add("bg-blue-600/50","border","border-blue-400/60"),s.classList.remove("hover:bg-blue-600/30")):(s.classList.remove("bg-blue-600/50","border","border-blue-400/60"),s.classList.add("hover:bg-blue-600/30"))})}window.addEventListener("scroll",t),t(),e.forEach(i=>{i.addEventListener("click",function(r){r.preventDefault();const s=this.getAttribute("href").substring(1),n=document.getElementById(s);if(n){const a=n.offsetTop-80;window.scrollTo({top:a,behavior:"smooth"})}})})}
